...
/** Basic workflow instance engine implementation parameterized by effect type F[_]. */
trait BasicEngine[F[_]](using E: Effect[F]) extends WorkflowInstanceEngine[F] {
  ...
  override def handleSignal[Ctx <: WorkflowContext, Req, Resp](
      ...
      req: Req,
  ): F[SignalResult[WCEvent[Ctx], Resp]] = E.pure(workflow.handleSignal(signalDef)(req))
