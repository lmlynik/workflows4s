...
/** Engine that delegates all operations to another engine. */
trait DelegatingWorkflowInstanceEngine[F[_]] extends WorkflowInstanceEngine[F] {
  ...
  override def triggerWakeup[Ctx <: WorkflowContext](workflow: ActiveWorkflow[Ctx]): F[WakeupResult[WCEvent[Ctx]]] =
  ...
  override def handleSignal[Ctx <: WorkflowContext, Req, Resp](
      ...
      req: Req,
  ): F[SignalResult[WCEvent[Ctx], Resp]] = delegate.handleSignal(workflow, signalDef, req)

  override def handleEvent[Ctx <: WorkflowContext](workflow: ActiveWorkflow[Ctx], event: WCEvent[Ctx]): Option[ActiveWorkflow[Ctx]] =
    delegate.handleEvent(workflow, event)

  override def processEvent[Ctx <: WorkflowContext](workflow: ActiveWorkflow[Ctx], event: WCEvent[Ctx]): ActiveWorkflow[Ctx] =
