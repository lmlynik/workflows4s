...
/** Engine that registers workflow state changes with a registry.
  *
  * TODO check how this engine interacts with non-greedy mode. What happens when there is more to execute? What is
  * unexpected signal appears between wakeups?
  */
class RegisteringWorkflowInstanceEngine[F[_]](protected val delegate: WorkflowInstanceEngine[F], registry: WorkflowRegistry.Agent[F])(using
  ...
  override def triggerWakeup[Ctx <: WorkflowContext](workflow: ActiveWorkflow[Ctx]): F[WakeupResult[WCEvent[Ctx]]] = {
    E.flatMap(super.triggerWakeup(workflow)) { prevResult =>
      E.as(registeringRunningInstance(workflow, prevResult.hasEffect), prevResult)
    }
  }
