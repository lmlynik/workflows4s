...
object InterruptionBuilder {
  ...
  class Step0[Ctx <: WorkflowContext, F[_]]()(using val ctxEffect: Effect[F]) {
      ...
      class Step2[Evt <: WCEvent[Ctx]](signalHandler: (Input, Req) => F[Evt], evtCt: ClassTag[Evt]) {
        ...
        class Step3[Err, Out <: WCState[Ctx]](
          ...
          class Step4(responseBuilder: (Input, Evt) => Resp) {
            ...
            def named(operationName: String = null, signalName: String = null): WIO.Interruption[Ctx, F, Err, Out] =
              WIO.Interruption(source(Option(operationName), Option(signalName)), tpe)
            def autoNamed(using name: sourcecode.Name): WIO.Interruption[Ctx, F, Err, Out]                         =
              named(operationName = ModelUtils.prettifyName(name.value))
            def done: WIO.Interruption[Ctx, F, Err, Out]                                                           =
            ...
            private def source(operationName: Option[String], signalName: Option[String]): WIO[Input, Err, Out, Ctx] = {
              ...
              val handleSignal: WIO.HandleSignal[Ctx, F, Input, Out, Err, Req, Resp, Evt] = WIO.HandleSignal(signalDef, sh, eh, meta)
              handleSignal
            }
    ...
    class TimeoutInterruptionStep1(durationSource: WIO.Timer.DurationSource[Input]) {
      ...
      case class Step2(private val startedEventHandler: EventHandler[Input, Unit, WCEvent[Ctx], Timer.Started]) {
        ...
        case class Step3(
          ...
          def named(timerName: String): WIO.Interruption[Ctx, F, Nothing, WCState[Ctx]]               = WIO.Interruption(source(timerName.some), tpe)
          def autoNamed(using name: sourcecode.Name): WIO.Interruption[Ctx, F, Nothing, WCState[Ctx]] = named(ModelUtils.prettifyName(name.value))
          def done: WIO.Interruption[Ctx, F, Nothing, WCState[Ctx]]                                   = WIO.Interruption(source(None), tpe)
